...
auction_to_check <- '17017700000420150012'
...
plot_int <- df_intervalos %>% 
  ...
  filter(data_hora > lubridate::ymd_hms('2015-04-07 10:28:00')) %>%
  # filter(segundos < 40) %>% # <<<<
  ...
  labs(
    ...
    ii) Para melhor visualização, são exibidos apenas lances registrados após às 10:28.'
  ) +
...
plot_desc <- df_bids %>% 
  ...
  scale_shape_manual(values = c(16, 4), name = 'CNPJ') +
  scale_color_manual(values = c('red', 'blue'), name = 'CNPJ') +
  scale_y_continuous(breaks = c(-0.01, -seq(3, 30, by = 3)), labels = 

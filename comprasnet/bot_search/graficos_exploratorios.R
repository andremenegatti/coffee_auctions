...
auction_to_check <- '15819800000320130014'
...
df_intervalos <- df_bids %>% 
  select(CNPJ_CPF, desconto_bruto, data_hora, intervalo_lance_anterior,
         intervalo_menor_lance,intervalo_lance_proprio) %>% 
  pivot_longer(cols = contains('intervalo_'), names_prefix = 'intervalo_',
               names_to = 'tipo_intervalo', values_to = 'segundos') %>% 
  mutate(tipo_intervalo = str_replace(tipo_intervalo, '_', ' ') %>%
           str_replace('proprio', 'próprio') %>% 
           tools::toTitleCase() %>% 
...
plot_int <- df_intervalos %>% 
  ...
  facet_grid(CNPJ_CPF ~ tipo_intervalo, scales = 'free') ; plot_int
...
plot_desc <- df_bids %>% 
  ...
  ggplot(aes(x = data_hora, y = desconto_bruto,
             col = CNPJ_CPF, shape = CNPJ_CPF)) + 
  geom_point(alpha = 0.5) +
  scale_shape_manual(values = c(16, 4), name = 'CNPJ') +
  scale_color_manual(values = c('red', 'blue'), name = 'CNPJ') +

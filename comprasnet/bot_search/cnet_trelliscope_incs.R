...
df_plot_data <- df_lances_completo  %>%
  nest(bids = c(valor_lance:incremento_lance_proprio_bruto)) %>% 
  mutate(mes = lubridate::month(abertura_lances) %>% as.factor())

# Joining dados do leilão
df_plot_data2 <- df_plot_data %>% 

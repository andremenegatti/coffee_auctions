...
# Abrindo bases ---------------------------------------------------------------
bec_cafe <- readRDS('ajustes-marcas/bec_cafe_dd_main13.rds') %>% 
  mutate(num_forn_aleat = num_forn_lances) %>% 
  filter(win_bid_kg < 80) # Removendo 2 observações (possíveis erros)

cnet_cafe <- readRDS('ajustes-marcas/cnet_cafe_dd_main13.rds') %>% 
  PregoesBR::trim_df('win_bid_kg', 1.5)

cnet_cafe_sp <- readRDS('ajustes-marcas/cnet_sp_cafe_dd_main13.rds')
...
# Salvando --------------------------------------------------------------------
saveRDS(dd_data_list$dd_brasil, 'ajustes-marcas/dd_brasil_main13.rds')
saveRDS(dd_data_list$dd_sp, 'ajustes-marcas/dd_sp_main13.rds')
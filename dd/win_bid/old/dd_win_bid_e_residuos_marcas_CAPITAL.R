...
bec_municipios_pregoes <- readRDS("~/Mestrado/Dissertacao/Auctions/BEC/bec_municipios_pregoes.rds") %>%
  inner_join(readRDS("~/Mestrado/Dissertacao/Auctions/BEC/bec_municipios_geocoded.rds"), by = c('municipio' = 'id'))
...
BEC_cafe <- readRDS('BEC/BEC_cafe_etapa4.rds') %>%
  ...
  left_join(bec_municipios_pregoes, by = 'id_item')
...
# Filtrando capital
cnet_cafe_capital <- cnet_cafe_sp %>%
  filter(municipio_uasg == 'SAO PAULO')

BEC_cafe_capital <- BEC_cafe %>%
  filter(municipio == 'SAO PAULO')

summary(trim_df(BEC_cafe_capital, 'win_bid', perc = 5) %>% select(win_bid) %>% unlist())
summary(trim_df(cnet_cafe_capital, 'win_bid', perc = 5) %>% select(win_bid) %>% unlist())

# Trimming (NECESSARIO APENAS EM WIN_BID)
BEC_cafe <- trim_df(BEC_cafe, 'win_bid')
BEC_cafe_capital <- trim_df(BEC_cafe_capital, 'win_bid')
cnet_cafe <- trim_df(cnet_cafe, 'win_bid')
cnet_cafe_sp <- trim_df(cnet_cafe_sp, 'win_bid')
cnet_cafe_capital <- trim_df(cnet_cafe_capital, 'win_bid')

#### CONSTRUINDO BASES PARA O PRIMEIRO ESTAGIO ####

# Selecionando variaveis
data_list <- list(BEC_cafe, BEC_cafe_capital, cnet_cafe, cnet_cafe_sp, cnet_cafe_capital) %>%
  map(.f = ~ .x %>%
  ...
  set_names(c('BEC_cafe_1st_stage', 'capital_BEC_cafe_1st_stage', 'cnet_cafe_1st_stage', 'sp_cnet_cafe_1st_stage', 'capital_cnet_cafe_1st_stage'))
...
m1_bec_capital <- lm(win_bid ~ marca_vencedor_principais, data = capital_BEC_cafe_1st_stage)
capital_BEC_cafe_1st_stage$res_m1 <- m1_bec_capital$residuals
...
m1_cnet_capital <- lm(win_bid ~ marca_vencedor_principais, data = capital_cnet_cafe_1st_stage)
capital_cnet_cafe_1st_stage$res_m1 <- m1_cnet_capital$residuals
...
m2_bec_capital <- lm(win_bid ~ marca_vencedor_principais + unidade_compradora, data = capital_BEC_cafe_1st_stage)
capital_BEC_cafe_1st_stage$res_m2 <- m2_bec_capital$residuals
...
m2_cnet_capital <- lm(win_bid ~ marca_vencedor_principais + unidade_compradora, data = capital_cnet_cafe_1st_stage)
capital_cnet_cafe_1st_stage$res_m2 <- m2_cnet_capital$residuals
...
# Apenas Capital
capital_cafe_dd <- bind_rows(capital_BEC_cafe_1st_stage, capital_cnet_cafe_1st_stage) %>%
  mutate(treat1 = if_else(comprasnet == 1 & abertura_lances >= data_20s & abertura_lances < data_3s, 1, 0),
         treat2 = if_else(comprasnet == 1 & abertura_lances > data_3s, 1, 0),
         bimestre = factor(inicio_bimestre),
         mes = factor(inicio_mes),
...
detach(capital_cafe_dd)

stargazer(dd_win_bid_bimestre_sp,
          dd_res_m1_bimestre_sp,
          dd_res_m1_bimestre_futuro_sp,
          dd_res_m1_bimestre_cepea_sp,
          dd_res_m1_bimestre_full_sp,
          omit = c("bimestre", "semana", "mes", "unidade_"),
          omit.stat = c("ser", "adj.rsq"),
          type = 'text',
          decimal.mark = ",",

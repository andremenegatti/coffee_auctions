...
trim_dict <- read_csv('dd/event/trim_dict_event2.csv')

df_br2 <- df_br %>% 
  mutate(sigla_uf = as.factor(sigla_uf)) %>% 
  add_custom_trimesters(trim_dict, baseline = 1) %>% 
  add_post_interactions(2:13) %>% 
...
df_plot %>% 
  ...
  # geom_label(x = data_3s, y = -0.15, label = 'Implementação\nRegra 3s',
             # family = 'serif', size = 3) +
  scale_x_date(breaks = df_dates$date_first_auction[-2]) +

...
df_m1_sp <- df_sp %>% 
  select(log_win_bid, starts_with("cnet_X_trim"),
...
lm_model <- lm(log_win_bid ~ 
                 ...
                 cnet_X_trim_post_19 +
                 cnet_X_trim_post_20 +
                 qualidade + kg_por_unid + arab_defl + futuro_defl +
                 trim_factor + unidade_compradora + municipio,
               data = df_m1_sp)

# trim_dict <- read_csv('dd/placebo/trim_dict_event.csv')

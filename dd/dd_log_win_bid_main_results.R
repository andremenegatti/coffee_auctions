...
df_models_sp <- df_models_sp %>% 
  mutate(lm_models = map(.x = formula,
                         .f = ~ str_replace(.x, '\\|', '+') %>% 
                           lm(data = dd_data_list$dd_sp)),
    hc1_se = map(.x = lm_models,
                 .f = ~ PregoesBR::get_robust_std_errors(.x, HC = 'HC1') %>% 
                   head(n = 20))
    )
...
# Salvando dados
saveRDS(df_models_brasil, 'results/log_win_bid/main_results_brasil.rds')

...
# Rodando modelos para todas as combinacoes (pode demorar...)
placebo_cross <- placebo_cross %>%
  ...
  # Extraindo dados dos resultados e criar colunas com coefs e erros padrao
  PregoesBR::get_tidy_estimates()

# Heatmaps / Tiles : Regra dos 20s --------------------------------------------
tiles_treat1 <- ggplot(placebo_cross) +
  ...
  labs(
    ...
    subtitle = 
      'Grade com datas de implementação alternativas para ambas as regras',
...
p_3s <- plot_ly() %>%
  ...
  add_surface(x = ~bandwidth, y = ~bandwidth, z = ~treat2_lower_matrix,
              opacity = 0.3, name = 'Limite inferior - Int. Conf. 95%',
              showscale = FALSE, colorscale = 'Hot') %>%
  add_surface(x = ~bandwidth, y = ~bandwidth, z = ~ treat2_upper_matrix,
              opacity = 0.3, name = 'Limite superior - Int. Conf. 95%',
              showscale = FALSE, colorscale = 'Hot') %>%
...
htmlwidgets::saveWidget(p_3s, 'plots/placebo_3s_surface.html')

# 3D / Surface plot: Regra dos 20s---------------------------------------------
# Montando matrizes necessarias para o grafico - Regra 20s
treat1_matrix <- matrix(placebo_cross$treat1_est,
                        nrow = 121, ncol = 121, byrow = TRUE)

treat1_lower_matrix <- matrix(placebo_cross$treat1_lower,
                              nrow = 121, ncol = 121, byrow = TRUE)

treat1_upper_matrix <- matrix(placebo_cross$treat1_upper,
                              nrow = 121, ncol = 121, byrow = TRUE)
...
htmlwidgets::saveWidget(p_20s, 'plots/placebo_20s_surface.html')

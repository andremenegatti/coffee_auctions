...
# Alterando apenas Regra dos 3s -----------------------------------------------
# Usando a data verdadeira da regra dos 20s

# Definindo janela
bandwidth <- -180:180 # 180 dias antes e 180 depois
# Nota: como estamos variando apenas uma das datas, o problema fica
# computacionalmente mais leve; logo, fica facil usar uma janela maior

# Rodando modelos placebo
placebo_df_3s <-
  # Criando DF com coluna com datas alternativas da regra dos 3s
  tibble(data_3s_alt = bandwidth + data_3s) %>%
  # Adicionando dummies de tratamento considerando as datas alternativas
  mutate(
    treat1 = map(
      .x = data_3s_alt,
      .f = ~ if_else(
  ...
  mutate(
    model_summary = map2(
      ...
      .y = treat2,
      .f = ~ lfe::felm(
        log_win_bid ~ .x + .y + qualidade + kg_por_unid + arab_defl | 
        ...
        broom::tidy() %>%
        # Substituindo '.x' por 'treat1' e '.y' por 'treat2' nos resultados
        mutate(term = case_when(term == '.x' ~ 'treat1',
                                term == '.y' ~ 'treat2',
                                !term %in% c('.x', '.y') ~ term))
    )
  ) %>%
  # Extraindo dados dos resultados e criando colunas com coefs e erros padrao
  PregoesBR::get_tidy_estimates()

# Grafico - Efeito Regra dos 3s
ggplot(data = placebo_df_3s) +
  geom_line(aes(x = data_3s_alt, y = treat2_est),
            linetype = 'solid', color = 'black') +
  geom_vline(xintercept = data_3s, color = 'darkred', linetype = 'dotted') +
  geom_line(aes(x = data_3s_alt, y = treat2_upper),
            linetype = 'dashed', color = 'black') +
  geom_line(aes(x = data_3s_alt, y = treat2_lower),
            linetype = 'dashed', color = 'black') +
...
ggplot(data = placebo_df_3s) +
  geom_line(aes(x = data_3s_alt, y = treat1_est),
            linetype = 'solid', color = 'black') +
  geom_vline(xintercept = data_3s, color = 'darkred', linetype = 'dotted') +
  geom_line(aes(x = data_3s_alt, y = treat1_upper),
            linetype = 'dashed', color = 'black') +
  geom_line(aes(x = data_3s_alt, y = treat1_lower),
            linetype = 'dashed', color = 'black') +
  geom_hline(yintercept = 0) +
...
# Alterando apenas Regra dos 20s ----------------------------------------------
...
placebo_df_20s <-
  ...
  # Adicionando dummy de tratamento considerando as datas alternativas
  # treat2 nao depende da regra dos 20s; precisamos criar apenas treat1
  mutate(
    treat1 = map(
      .x = data_20s_alt,
      .f = ~ if_else(
  ...
  mutate(
    model_summary = map(
      .x = treat1,
      .f = ~ lfe::felm(
        log_win_bid ~ .x + treat2 + qualidade + kg_por_unid + arab_defl | 
          bimestre + unidade_compradora + municipio + marca_vencedor_principais
      ) %>%
        # Extraindo resultados de forma limpa e organizada usando pacote broom
        broom::tidy() %>%
...
ggplot(data = placebo_df_20s) +
  geom_line(aes(x = data_20s_alt, y = treat1_est),
            linetype = 'solid', color = 'black') +
  geom_vline(xintercept = data_20s, color = 'darkred', linetype = 'dotted') +
  geom_line(aes(x = data_20s_alt, y = treat1_upper),
            linetype = 'dashed', color = 'black') +
  geom_line(aes(x = data_20s_alt, y = treat1_lower),
            linetype = 'dashed', color = 'black') +
  geom_hline(yintercept = 0) +
  labs(title = 'Teste Placebo - Efeito da Regra dos 20s',
...
# Grafico - Efeito Regra dos 3s
ggplot(data = placebo_df_20s) +
  geom_line(aes(x = data_20s_alt, y = treat2_est),
            linetype = 'solid', color = 'black') +
  geom_vline(xintercept = data_20s, color = 'darkred', linetype = 'dotted') +
  geom_line(aes(x = data_20s_alt, y = treat2_upper),
            linetype = 'dashed', color = 'black') +
  geom_line(aes(x = data_20s_alt, y = treat2_lower),
            linetype = 'dashed', color = 'black') +
  geom_hline(yintercept = 0) +
  labs(title = 'Teste Placebo - Efeito da Regra dos 3s',
       subtitle = 'Considerando datas alternativas para a regra dos 20s',

...
source('C:/Users/Dell/Desktop/Placebo/helpers_placebo.R')
...
df_placebo_pre <-
  tibble(data_placebo = seq(-620, -30, by = 10) + data_3s) %>%
  mutate(
    treat1 = map(
      .x = data_placebo,
  ...
  mutate(
    model_summary = pmap(
      ...
      .f = ~ lm(log_win_bid ~ ..1 + ..2 + ..3 + qualidade + kg_por_unid + futuro_defl + arab_defl + bimestre + unidade_compradora + municipio + marca_vencedor_principais)
    )
  )
...
df_placebo_full <- bind_rows(df_placebo_pre, df_placebo_post) %>%
...
df_placebo_full <- df_placebo_full %>%
...
ggplot(mapping = aes(x = data_placebo, group = grupo)) +
  ...
  labs(
    ...
    title = 'Teste Placebo - Efeito da Regra dos 3s',
...
ggsave('C:/Users/Dell/Desktop/placebo_3s_pre.png', width = 6, height = 9)





df_plot_post <-  df_placebo_full %>%
  select(-model, -standard_errors, -treat1, -treat2, -treat_placebo) %>%
  filter(data_placebo > data_3s) %>%
  gather(key = 'treat_var', value = 'coefficient', treat1_est:treat_placebo_lower) %>%
  mutate(grupo = str_remove(treat_var, 'treat|treat_') %>%
...
ggsave('C:/Users/Dell/Desktop/placebo_3s_post.png', width = 6, height = 9)

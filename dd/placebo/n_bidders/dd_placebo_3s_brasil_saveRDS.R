...
for (i in 1:nrow(df_placebo)) {
  ...
  model <- 
    lm(num_forn_lances ~ df_placebo$treat1[[i]] + 
         df_placebo$treat_placebo[[i]] + df_placebo$treat2[[i]] +
         qualidade + kg_por_unid + futuro_defl + 
         arab_defl + bimestre + sigla_uf:bimestre + 
         municipio + unidade_compradora)
  
  robust_se <- PregoesBR::get_robust_std_errors(model)
  
  saveRDS(robust_se, str_c('dd/placebo/data/n_bidders/brasil/',

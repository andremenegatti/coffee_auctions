...
for (i in seq(10, 69, by = 1)) {
  ...
  top100_uasgs_list <- data_list %>% 
    map(.f = ~ count(.x, unidade_compradora) %>% 
          arrange(desc(n)) %>% 
          slice(1:i) %>% 
  ...
  stargazer(df_fitted_models$models, type = 'text',
            out = str_c('dd/top_uasgs/n_leiloes/sp/txt/top', i, '.txt'))
  ...
  saveRDS(df_std_sp,
          str_c('dd/top_uasgs/n_leiloes/sp/hc1/top', i, '.rds'))
...
rob_est_list <- map(.x = top_seq,
    .f = ~ str_c('dd/top_uasgs/n_leiloes/sp/hc1/top', .x, '.rds') %>% 
